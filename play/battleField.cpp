...
namespace Play {
    ...
    BattleField::BattleField(GameMap* map_) 
     : SpellContext() {
        ...
        vector<void*> _rubbishBin = vector<void*>(0);
        vector<Combatable*> _playerAllied(0);
        vector<Combatable*> _nonPlayerAllied(0);
        _map = map_;
    }
     ...
     */
    vector<Mob*> BattleField::getDue(void) {
        vector<Mob*> result = vector<Mob*>(0);
        if (!isInCombat()) {
            return result;
        }
