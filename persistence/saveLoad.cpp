...
namespace Persistence {
    ...
    void SaveLoad::save(const Party& party) {
        ...
        for (PC* pc : party.members()) {
            ...
            vector<byte> spells = getSpellBytes(*pc);
    ...
    void SaveLoad::pushNumeric(vector<byte>& data, unsigned short value) {
        vector<byte> bytes = Util::splitShort(value);
        data.insert(data.end(), bytes.begin(), bytes.end());
    }

    vector<byte> SaveLoad::getSpellBytes(const PC& pc) const {
    ...
    void SaveLoad::load(Party& party) const {
        ...
        vector<Spell*> spells = vector<Spell*>();
