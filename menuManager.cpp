...
namespace Play {
    ...
    bool MenuManager::moveCursorSideways(Party& party, Core::InputPress input) {
        ...
        vector<Command*> spells = party.memberAt(_selectedMemberIndex)->spells();
        if (
            (_selectedSpellIndex >= 0 && spells.size() <= natural(_selectedSpellIndex)) 
            || !spells.at(_selectedSpellIndex)->isValid()
        ) {
            return false;
        }
    ...
    bool MenuManager::moveCursor(Party& party, InputPress input) {
        if (state() == MenuState::SelectSpell && in(input, InputPress::LEFT, InputPress::RIGHT)) {
    ...
    bool MenuManager::processSpellCommand(Party& party)
    {
        ...
        vector<Command*> spells = party.memberAt(_selectedMemberIndex)->spells();
        if (
            int(spells.size()) <= _selectedSpellIndex 
            || spells.at(_selectedSpellIndex)->components().size() <= 0
        ) {
            _selectedRuneIndex = 0;
            state(MenuState::SelectRune);
        } else {
    ...
    bool MenuManager::processRuneCommand(const Party& party) {
        ...
        if (int(pc->spells().size()) <= _selectedSpellIndex) {
            pc->addSpell(new Spell(vector<Rune*>(0)));
        }
    ...
    bool MenuManager::cast(Play::Party& party, Play::Mob& caster, int spellIndex) {
        ...
        SpellContext context = SpellContext(party.members(), vector<Enemy*>());
